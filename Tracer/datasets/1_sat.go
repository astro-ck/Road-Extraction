...
func main() {
	...
	for _, tile := range requiredTiles {
		...
		for xOffset := 0; xOffset < 1024; xOffset += 512 {
			for yOffset := 0; yOffset < 1024; yOffset += 512 {
				centerWorld := tile.Region.CenterWorld.Add(common.Point{float64(tile.X*1024 + xOffset), float64(-(tile.Y*1024 + yOffset))}.Scale(widthWorld))
